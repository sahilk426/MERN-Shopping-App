...
const lodMemo = require('lodash.memoize');
...
class Color {
    static fromHex(hex) {
        return Format.parseHex(hex) || Color.red;
    }

    static fromArray(arr) {
        return new Color(new RGBA(arr[0], arr[1], arr[2], 1));
    }
...
class Format {
    static formatRGB(color) {
        ...
        return Format.formatRGBA(color);
    }
     ...
     */
    static formatHexA(color, compact = false) {
        if (compact && color.rgba.a === 1) {
            return Format.formatHex(color);
        }
     ...
     */
    static parseHex = lodMemo((hex, getColor = false) => {
        ...
        if (length === 5) {
            ...
            arr = [16 * r + r, 16 * g + g, 16 * b + b, (16 * a + a) / 255];
        }

        if (arr.length) {
            if (getColor) {
                return new Color(new RGBA(...arr));
            }
