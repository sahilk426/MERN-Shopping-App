...
const TIME_LABEL_PATTERN = /^(\d+y)?\s*(\d{1,3}d)?\s*(\d{1,3})?(:\d{1,3})?(:\d{3})?$/i;
 ...
 * @returns {void}
 */
const MetricsProvider = function MetricsProvider(stats) {
    /**
    ...
    const setupAggregation = function setupAggregation() {
        // construct the aggregation container

        this._aggregation = reduce(
        ...
        this.highestAggregationKey = this.aggregationLevels[this.aggregationLevels.length - 1];
    ...
    this.adjustScrollOffsetThrottled = throttle(this.adjustScrollOffset.bind(this), 75, {
        leading: true,
    });

    this.adjustZoomLevelThrottled = throttle(this.adjustZoomLevel.bind(this), 75, {
        leading: true,
    });
    ...
    // callback handlers
    stats.on('metrics', this._onMetrics.bind(this));
    stats.on('zoomGraphs', this.adjustZoomLevelThrottled);
    stats.on('scrollGraphs', this.adjustScrollOffsetThrottled);
    stats.on('startGraphs', this.startGraphs.bind(this));
    stats.on('resetGraphs', this.resetGraphs.bind(this));
};
 ...
 */
const convertElapsedTimeToTimeIndex = function convertElapsedTimeToTimeIndex(
    currentTime,
    startTime,
    aggregateTimeUnits
) {
...
MetricsProvider.prototype.setZoomLevel = function setZoomLevel(zoom) {
    this.zoomLevel = clamp(zoom, 0, this.aggregationLevels.length - 1);
...
MetricsProvider.prototype.adjustZoomLevel = function adjustZoomLevel(zoom) {
    ...
    const newLevel = this.zoomLevel + zoom;

    if (this.zoomLevel !== newLevel || this.init) {
...
MetricsProvider.prototype.adjustScrollOffset = function adjustScrollOffset(scroll, absolute) {
    ...
    newOffset = Math.min(currentAggregation.scrollOffset + scroll, 0);

    if (currentAggregation.scrollOffset !== newOffset || this.init) {
        currentAggregation.scrollOffset = newOffset;
        this.emit('refreshMetrics');
    }
};
*/
...
MetricsProvider.prototype.adjustScrollOffset = function adjustScrollOffset(scroll, absolute) {
    ...
    // apply the offset (but do not go above zero)
    currentAggregation.scrollOffset = Math.min(currentAggregation.scrollOffset + scroll, 0);
 ...
 */
const getInitializedAverage = function getInitializedAverage(data) {
    if (!data) {
        return {};
    }
    ...
    return reduce(
        ...
        (prev, a, dataKey) => {
            ...
            forEach(data[dataKey], (b, dataMetricKey) => {
 ...
 * @returns {void}
 */
const aggregateMetrics = function aggregateMetrics(currentTime, metricData) {
    let aggregateKey;
     ...
     * @returns {void}
     */
    const setAggregateData = function setAggregateData(index, data) {
     ...
     * @returns {void}
     */
    const addMissingTimeSlots = function addMissingTimeSlots(currentTimeIndex) {
     ...
     * @returns {void}
     */
    const getAveragedAggregate = function getAveragedAggregate(rows, startIndex, endIndex) {
        ...
        for (const dataKey in metricData) {
            for (const dataMetricKey in metricData[dataKey]) {
                ...
                averagedAggregate[dataKey][dataMetricKey] = Number(
    ...
    const processRow = function processRow(rowIndex, rows) {
        ...
        const currentTimeIndex = convertElapsedTimeToTimeIndex(
            currentTime,
            this._startTime,
            Number(aggregateKey)
        );
 ...
 * @returns {void}
 */
MetricsProvider.prototype._onMetrics = function _onMetrics(data) {
 ...
 */
MetricsProvider.prototype.getMetrics = function getMetrics(limit) {
    const currentAggregation = this.getCurrentAggregation();
     ...
     */
    const getFixedScrollOffset = function getFixedScrollOffset(offset, length) {
    ...
    currentAggregation.scrollOffset = getFixedScrollOffset(
        currentAggregation.scrollOffset,
        currentAggregation.data.length
    );
...
MetricsProvider.prototype.getXAxis = function getXAxis(limit) {
    ...
    for (let timeIndex = -scrollOffset + limit - 1; timeIndex >= -scrollOffset; timeIndex--) {
 ...
 */
MetricsProvider.prototype.validateTimeLabel = function validateTimeLabel(label) {
    ...
    if (isNaN(timeValue) || !inRange(timeValue, 0, timeRange.maxTime.value + 1)) {
        throw new Error(
            `Enter a time value between ${timeRange.minTime.label} and ${timeRange.maxTime.label}`
        );
    }
