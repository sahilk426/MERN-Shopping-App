...
const TIME_LABEL_PATTERN = /^(\d+y)?\s*(\d{1,3}d)?\s*(\d{1,3})?(:\d{1,3})?(:\d{3})?$/i; // constants.TIME_LABEL_PATTERN;
 ...
 */
exports.getLabel = function getLabel(timeValue) {
    ...
    TIME_SCALES.every((timeScale, index, timeScales) => {
        //every(TIME_SCALES, (timeScale, index, timeScales) => {
        const timeElement = {
    ...
    return reduce(
        ...
        (prev, curr, index) => {
            switch (curr.units) {
                case 's':
                    return `:${`${curr.value}`.padStart(DIGITS_PER_UNIT, '0')}`;
                case 'm':
                case 'h':
                    if (index < timeElements.length - 1) {
                        return (
                            (curr.units === 'm' ? ':' : ' ') +
                            `${curr.value}`.padStart(DIGITS_PER_UNIT, '0') +
                            prev
                        );
                    }
...
exports.convertTimeLabelToMilliseconds = function (label) {
    ...
    forEach(split.slice(1), (value, index) => {
